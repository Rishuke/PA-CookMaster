...
if(
	...
	header('location: register.php?message=Vous devez remplir tous les champs.&type=danger');
	exit;
}
// Si email invalide > redirection vers le formulaire avec un paramètre get "message" : "Email invalide."
if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
	header('Location: register.php?message=emailErr.&type=danger');
	exit;
}
...
if(strlen($_POST['password']) < 6 || strlen($_POST['password']) > 12){
	header('Location: register.php?message=passwordErr.&type=danger');
	exit;
}
...
if(!empty($results)){
	...
	header('Location: register.php?message=alreadyEmail&type=danger');
}
...
if($_FILES['image']['error'] != 4){ // un fichier a été envoyé
	...
	if(!in_array($_FILES['image']['type'], $acceptable)){
		header('Location: register.php?message=fileTypeErr&type=danger');
	}
	
	//verifier que le fichier moins de 2Mo, si non : redirection
	$maxSize = 2 * 1024 * 1024; // 2Mo
	if($_FILES['image']['size'] > $maxSize){
		header('Location: register.php?message=fileSize&type=danger');
	}
...
if(!$succes){
	// Redirection vers l'register avec un message
	header('Location: register.php?message=L\'enregistrement a échoué&type=danger');
}


// Redirection vers l'accueil avec un message
header('Location: register.php?message=Vous êtes bien enregistré sur Wicookin&type=success');
